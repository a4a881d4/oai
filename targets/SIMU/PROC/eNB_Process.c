...
main (int argc, char **argv)
{
  ...
  int port=38800,node_id=0,Process_Flag=0,wgt,Channel_Flag=0;
  char c;
  s32 i, j;
  int new_omg_model; // goto ocg in oai_emulation.info.
  // pointers signal buffers (s = transmit, r,r0 = receive)
  //double **s_re, **s_im, **r_re, **r_im, **r_re0, **r_im0;
  double **s_re2[NUMBER_OF_eNB_MAX+NUMBER_OF_UE_MAX], **s_im2[NUMBER_OF_eNB_MAX+NUMBER_OF_UE_MAX], **r_re2[NUMBER_OF_eNB_MAX+NUMBER_OF_UE_MAX], **r_im2[NUMBER_OF_eNB_MAX+NUMBER_OF_UE_MAX], **r_re02, **r_im02;
  double **r_re0_d[NUMBER_OF_UE_MAX][NUMBER_OF_eNB_MAX], **r_im0_d[NUMBER_OF_UE_MAX][NUMBER_OF_eNB_MAX], **r_re0_u[NUMBER_OF_eNB_MAX][NUMBER_OF_UE_MAX],**r_im0_u[NUMBER_OF_eNB_MAX][NUMBER_OF_UE_MAX];
  ...
  while ((c = getopt (argc, argv, "haePToFt:C:N:k:x:m:rn:s:S:f:z:u:b:c:M:p:g:l:d:U:B:R:E:i:X:"))
	 != -1) {
    ...
    case 'i':
     Process_Flag=1;
     node_id =wgt+atoi(optarg); //[ALU]
     port=38800+wgt+atoi(optarg); //[ALU]
     break;
    default:
      help ();
      exit (-1);
...
//  transmission_mode_rrc = oai_emulation.info.transmission_mode;//FIXME!!!
  ...
  init_mmap(node_id,frame_parms, &(s_re2[0]), &(s_im2[0]), &(r_re2[0]), &(r_im2[0]), &(r_re02[0]), &(r_im02[0]));
  }
  ...
  //ALU Individual Process
  if(Process_Flag==1){
   Process_Func(node_id,port,r_re02,r_im02,r_re2[0],r_im2[0],s_re2[0],s_im2[0],eNB2UE,UE2eNB,enb_data,ue_data,abstraction_flag,frame_parms);
  }
...
//    oai_emulation.info.frame = mac_xface->frame;
  //  update_nodes(oai_emulation.info.time);
	...
	if (mac_xface->frame >= (UE_id * 10)) {	// activate UE only after 10*UE_id frames so that different UEs turn on separately
      ...
      if (direction  == SF_DL) {
	do_DL_sig(r_re02,r_im02,r_re2,r_im2,s_re2,s_im2,eNB2UE,enb_data,
	...
	*/
	...
	do_UL_sig(r_re02,r_im02,r_re2,r_im2,s_re2,s_im2,UE2eNB,next_slot,abstraction_flag,frame_parms);
      }
      else {//it must be a special subframe
	if (next_slot%2==0) {//DL part
	  do_DL_sig(r_re02,r_im02,r_re2,r_im2,s_re2,s_im2,eNB2UE,enb_data, ue_data,next_slot,abstraction_flag,frame_parms);
	  /*
	    for (aarx=0;aarx<UE2eNB[1][0]->nb_rx;aarx++)
	    for (aatx=0;aatx<UE2eNB[1][0]->nb_tx;aatx++)
	    for (k=0;k<UE2eNB[1][0]->channel_length;k++)
