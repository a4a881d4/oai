...
void do_DL_sig_channel_T(void *param){
	...
	eNB_i=(cthread->eNB_id);
  	...
  	enb_data[eNB_i]=cthread->enb_data;
  	...
  	s32 att_eNB_id=-1;
  	  ...
  	  while(1){
  		...
  		printf("[SIM][DL] UE %d : ADC in (eNB %d) %f dB for slot %d (subframe %d)\n",UE_i,eNB_i,10*log10(rx_pwr),next_slot,next_slot>>1);

  		count=0;
		count = __sync_add_and_fetch(&_COT, 1);
	  	COMPILER_BARRIER();
	 // 	printf("NUM_THREAD_DOWNLINK %d \t COT %d\t Thread Id %d\n",NUM_THREAD_DOWNLINK,count,thread_id);
	  	//COT++;
...
void alpha_channel(int eNB_id,int UE_id,double **r_re,double **r_im,double **r_re0,double **r_im0,LTE_DL_FRAME_PARMS *frame_parms)
{
	  u32 i,aa;
	  double alpha=0.0;
...
void do_UL_sig_channel_T(void *param){
	...
	eNB_i=(cthread->eNB_id);
  	...
  	s32 **txdata,**rxdata;
    	...
    	if(pthread_cond_wait(&uplink_cond[UE_i][eNB_i], &uplink_mutex[UE_i][eNB_i])) exit(1);//[UE_i][eNB_i]);
    		//  pthread_cond_wait(&uplink_cond[UE_i][eNB_i], &uplink_mutex[0][0]);
  		//pthread_mutex_lock(&exclusive);
  		multipath_channel(UE2eNB[UE_i][eNB_i],s_re,s_im,r_re0,r_im0,
  				  frame_parms->samples_per_tti>>1,0);
  		...
  		rf_rx(r_re0,
  	  	...
  	  	COMPILER_BARRIER();
	 // 	printf("NUM_THREAD_UPLINK %d \t COT_U %d\t thread_id %d\n",NUM_THREAD_UPLINK,count,thread_id);
  	  	//COT_U++;
  //	  if (pthread_mutex_lock(&uplink_mutex_channel)) exit(1);
  	...
  	//    		if(pthread_mutex_unlock(&uplink_mutex_channel)) exit(1);
  	    //if(count==NUM_THREAD_UPLINK){
  	    //pthread_cond_signal(&uplink_cond_channel);
  	    //}
  	    		//pthread_mutex_unlock(&exclusive);
    	  }
