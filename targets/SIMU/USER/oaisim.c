...
main (int argc, char **argv)
{
  ...
  while ((c = getopt (argc, argv, "haePToFt:C:N:k:x:m:rn:s:S:f:z:u:b:c:M:p:g:l:d:U:B:R:E:X:i:"))
	 != -1) {
  ...
  if (abstraction_flag == 0 && Process_Flag==0 && Channel_Flag==0) //ALU MODIFIED
...
printf("------------------ Frame %d\n -------------------------",mac_xface->frame);
	...
	*/
    ...
    if ((mac_xface->frame==1)&&(abstraction_flag==0)&&(Channel_Flag==0)) { //ALU
